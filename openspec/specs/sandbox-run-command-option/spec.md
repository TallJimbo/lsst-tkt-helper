# sandbox-run-command-option

## Purpose

Overriding the final command emitted by `tkt sandbox-run` via a `--command`
option, so the user can run e.g. opencode's TUI instead of the configured ACP
server without editing configuration files.
