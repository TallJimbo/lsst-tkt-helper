...
Pre-installing the hook environments for a package's pre-commit (or prek)
configuration at setup time, so that an agent's first commit in a
network-restricted sandbox runs the hooks offline from the shared store.
(TBD: expand.)
