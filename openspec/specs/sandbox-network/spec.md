...
Restricting the network access of the `tkt` sandbox by default, exposing a
single bridged localhost port, while allowing full network access via an
opt-in flag. (TBD: expand.)
